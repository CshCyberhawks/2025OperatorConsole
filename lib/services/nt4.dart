...
class NT4Client {
  ...
  NT4Client({
    ...
    _timeSyncBgEvent = Timer.periodic(const Duration(milliseconds: 5000), (
      timer,
    ) {
      _wsSendTimestamp();
    });
...
class NT4Topic {
  ...
  Map<String, dynamic> toPublishJson() {
    return {'name': name, 'type': type, 'pubuid': pubUID};
  }
...
class NT4ValueReq {
  ...
  const NT4ValueReq({this.topics = const []});
